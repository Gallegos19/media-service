...
export class MediaController {
  ...
  public getFilesByCategory = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      res.status(200).json({
        success: true,
        data: files.map((file: MediaFile) => this.formatMediaFileResponse(file))
      });
    } catch (error) {
      next(error);
    }
  ...
  public getFiles = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Use userId from query params if available, otherwise use from path params
  ...
  public uploadFile = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      console.error('Error en el controlador uploadFile:', {
        error: error,
        message: errorMessage,
        stack: errorStack
      });
      ...
      if (error instanceof Error) {
        ...
        next(new Error('Ocurrió un error desconocido'));
      }
    }
  };
  ...
  public getFile = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Verificar que el usuario sea el propietario o el archivo sea público
      if (mediaFile.uploadedByUserId !== userId && !mediaFile.isPublic) {
        return res.status(403).json({
          success: false,
          error: 'No tienes permiso para ver este archivo',
        });
  ...
  public deleteFile = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      await this.mediaStorageService.deleteMedia(mediaFile, userId);
  ...
  // Formatear la respuesta de un archivo
  ...
  // Formatear la respuesta de una variante
